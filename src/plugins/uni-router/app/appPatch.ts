...
export function registerLoddingPage(router: Router): void {
...
export function runtimeQuit(title: string | undefined = '再按一次退出应用'): void {
  ...
  } else if (nowTime - quitBefore < 1000) {
    plus.runtime.quit()
  }
}
...
export function tabIndexSelect(to: totalNextRoute, from: totalNextRoute): boolean {
  if (!(__uniConfig.tabBar && Array.isArray(__uniConfig.tabBar.list))) {
    return false
  }

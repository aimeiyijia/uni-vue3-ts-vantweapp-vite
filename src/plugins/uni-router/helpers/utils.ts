...
export function def(defObject: objectAny, key: string, getValue: Function) {
...
export function assertNewOptions<T extends InstantiateConfig>(options: T): T | never {
...
export function getWildcardRule(router: Router, msg?: navErrorRule): RoutesRule | never {
...
export function notRouteTo404(
  ...
  const redirectRule = navjump(
    newRoute as totalNextRoute,
    router,
    navType,
    undefined,
    undefined,
...
export function removeSimpleValue(array: Array<string | number>, value: string): Boolean {
  ...
  if (getDataType<totalNextRoute | string>(toRule) === '[object Object]') {
    ...
    if (name != null && paramsQuery != null) {
      ...
      if (finallyPath.includes(':')) {
        ...
        ERRORHOOK[0](
          { type: 2, msg: `动态路由：${finallyPath} 无法使用 paramsToQuery！`, toRule },
          router
        )
      } else {
        return {
...
export function baseClone<
  ...
  K extends keyof T
>(source: T, target: Array<any> | objectAny): Array<any> | objectAny | null {
  ...
  if (source == null) {
    ...
    for (const key of Object.keys(source)) {
      ...
      if (typeof source[key] === 'object') {
        target[dyKey] =
          getDataType<T>(source[key]) === '[object Array]' ? ([] as Array<any>) : ({} as objectAny)
...
export function deepClone<T>(source: T): T {
  const __ob__ =
    getDataType<T>(source) === '[object Array]' ? ([] as Array<any>) : ({} as objectAny)
...
export function assertParentChild(parentPath: string, vueVim: any): boolean {
...
export function resolveAbsolutePath(path: string, router: Router): string | never {
  const reg = /^\/?([^\?\s]+)(\?.+)?$/
  ...
  if (/^\.\/[^\.]+/.test(trimPath)) {
    ...
    const navPath: string =
      (router as unknown as { currentRoute: { path: string } }).currentRoute.path + path
    return navPath.replace(/[^\/]+\.\//, '')
  }
  const relative = paramsArray[1]
    .replace(/\//g, `\\/`)
    .replace(/\.\./g, `[^\\/]+`)
    .replace(/\./g, '\\.')
...
export function deepDecodeQuery(query: objectAny): objectAny {

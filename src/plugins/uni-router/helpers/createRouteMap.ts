...
export function createRouteMap(router: Router, routes: RoutesRule[]): routesMapRule | never {
  ...
  routes.forEach((route) => {
    ...
    if (path == null) {
      throw new Error(
        `请提供一个完整的路由对象，包括以绝对路径开始的 ‘path’ 字符串 ${JSON.stringify(route)}`
      )
    }
    if (finallyPath instanceof Array) {
      if (!(router.options.h5 as H5Config).vueRouterDev && router.options.platform === 'h5') {
        throw new Error(
          `非 vueRouterDev 模式下，route.alias 目前无法提供数组类型！ ${JSON.stringify(route)}`
        )
      }
    }

...
import { buildVueRouter, buildVueRoutes } from '../H5/buildRouter'
import { proxyEachHook } from '../H5/proxyHook'
import { createRouteMap } from '../helpers/createRouteMap'
import { pageTypeRule, Router, routesMapRule, RoutesRule } from '../options/base'
...
let registerRouter = false
let onloadProxyOk = false
  ...
  const toggleHooks = {
    ...
    'app-lets': {
      beforeCreate(this: any): void {
        ...
        // 保证这个函数不会被重写
        const pluginMark = npmPackageInfo.name
        voidFun(pluginMark)

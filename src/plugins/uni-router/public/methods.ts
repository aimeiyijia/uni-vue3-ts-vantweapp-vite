...
import { parseQuery, queryPageToMap, resolveQuery } from './query'
  ...
  if (router.options.platform === 'h5') {
    ...
    if (nextCall != null) {
      ...
      if (navType === 'push' && Reflect.has(parseToRule, 'events')) {
        ...
        ;(router.$route as any)[navType](
          parseToRule,
          (parseToRule as totalNextRoute).success || voidFun,
          (parseToRule as totalNextRoute).fail || voidFun
        )
      }
    }
  } else {
    ...
    transitionTo(router, parseToRule, from, navType, HOOKLIST, function (callOkCb: Function): void {
...
export function createRoute(
  ...
  if (router.options.platform === 'h5') {
    ...
    const toQuery = parseQuery(
      { ...matRouteParams, ...copyData(vueRoute.query as objectAny) },
      router
    )
    ...
    if (orignRule != null) {
      ...
      if (Object.keys(page).length === 0) {
        ...
        const errorMsg = `不存在的页面栈，请确保有足够的页面可用，当前 level:${level}`
        ;(router.options.routerErrorEach as (error: navErrorRule, router: Router) => void)(
          {
            type: 3,
            msg: errorMsg,

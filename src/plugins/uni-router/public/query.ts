...
import { objectAny, Router, routesMapRule, RoutesRule, totalNextRoute } from '../options/base'
...
export function queryPageToMap(
    ...
    if (objNavRule.path != null) {
      ...
      if (route == null) {
        route = getWildcardRule(router, {
          type: 2,
          msg: `命名路由为：${objNavRule.name} 的路由，无法在路由表中找到！`,
          toRule
        })
      } else {
      ...
      route = getWildcardRule(router, {
        type: 2,
        msg: `${toRule} 解析失败，请检测当前路由表下是否有包含。`,
        toRule
      })
    }
  } else {
  ...
  if (router.options.platform === 'h5') {
    ...
    if (getDataType<Function | undefined>(completeCb) === '[object Function]') {
      const publicCb = function (
        this: any,
        args: Array<any>,
        callHook: Function | undefined
      ): void {
...
export function resolveQuery(toRule: totalNextRoute, router: Router): totalNextRoute {
...
export function parseQuery(query: objectAny, router: Router): objectAny {
  ...
  } else if (Reflect.get(query, 'query')) {
    // 验证一下是不是深度对象

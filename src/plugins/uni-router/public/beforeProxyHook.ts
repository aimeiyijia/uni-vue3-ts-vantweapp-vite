...
export function beforeProxyHook(Vim: any, router: Router): boolean {
  ...
  for (let i = 0; i < keyArray.length; i++) {
    ...
    if (hooksArray) {
      ...
      for (let j = 0; j < hooksArray.length; j++) {
        ...
        if (hookFun.toString().includes(npmPackageInfo.name)) {
          continue
        }
        const [oldHook] = hooksArray.splice(
          j,
          1,
          function myReplace(this: any, ...args: Array<any>) {
            const pluginMarkId = npmPackageInfo.name
      ...
      warn(
        `beforeProxyHooks ===> 当前组件不适合${key}，或者 hook: ${key} 不存在，已为你规避处理，可以忽略。`,
        router
      )
    }
  }

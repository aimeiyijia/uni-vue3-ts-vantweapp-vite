...
import { deepClone, getDataType, getUniCachePage } from '../helpers/utils'
import { objectAny, pageTypeRule, Router, totalNextRoute, vueOptionRule } from '../options/base'
import { createRoute } from './methods'
import { stringifyQuery } from './query'

export function createToFrom(to: totalNextRoute, router: Router): totalNextRoute {
...
export function createFullPath(to: totalNextRoute, from: totalNextRoute): void {
  if (to.fullPath == null) {
...
export function proxyPageHook(vueVim: any, router: Router, pageType: pageTypeRule): void {
  ...
  for (let i = 0; i < proxyHookName.length; i++) {
    ...
    if (hookList) {
      for (let k = 0; k < hookList.length; k++) {
        ...
        if (originHook.toString().includes(npmPackageInfo.name)) {
          continue
        }
        ...
        hookDeps.hooks[resetIndex] = {
          ...
          callHook: (enterPath: string): void => {
            if (
              router.enterPath.replace(/^\//, '') !== enterPath.replace(/^\//, '') &&
              pageType !== 'app'
            ) {
              return
            }
...
export function resetPageHook(router: Router) {
  const proxyHookDeps = router.proxyHookDeps
